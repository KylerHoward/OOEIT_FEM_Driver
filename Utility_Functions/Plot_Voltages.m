...
% data should contain whatever you need to plot:
%   data.X{1}, data.Y{1}, data.Z{1}, data.C{1}, etc.
%   If flag.do_complex = true, also include {2}

%% --- Create UIFigure ---
fig = uifigure('Position',[100 100 700 700]);

%% --- Grid Layout ---
% Layout:
%   Row 1: plots (1 or 2)
%   Row 2: sliders (3 sliders stacked)
...
uislider(yaxisBlock, 'range',...
         'Limits',[min(nodes(:,1)), ceil(max(nodes(:,2)))],...
         'Value', [min(nodes(:,1)), ceil(max(nodes(:,2)))],...
         'Step', 5,...
         'ValueChangedFcn',@(src,evt) updateY(ax1, ax2, src.Value, flags));

%% --- Callback Functions ---
    function updateFrame(ax1, ax2, s1, s2, framenum, colordata, flags)
        frame = round(framenum);

        % Update plot 1
        s1.CData = real(colordata(:,flags.plot_volts,frame));
        title(ax1, sprintf("Frame %d\nReal Voltage: CP %d", frame, flags.plot_volts))

        % Update plot 2
        if flags.set_complex
            s2.CData = imag(colordata(:,flags.plot_volts,frame));
            title(ax2, sprintf("Frame %d\nReal Voltage: CP %d", frame, flags.plot_volts))
        end
    end

    function updateX(ax1, ax2, rangeVals, flags)
        ax1.XLim = [rangeVals(1), rangeVals(2)];
        if flags.set_complex
            ax2.XLim = [rangeVals(1), rangeVals(2)];
        end
    end

    function updateY(ax1, ax2, rangeVals, flags)
        ax1.YLim = [rangeVals(1), rangeVals(2)];
        if flags.set_complex
            ax2.YLim = [rangeVals(1), rangeVals(2)];
        end
    end