...
flags.CP_choice       = 2; % Choice of current pattern
    % 1: Standard pattern
...
carina_height = sbj_sheet.CarinaFromBottom_mm_(row_num);
T5_height     = sbj_sheet.T5FromBottom_mm_(row_num);
T8_height     = sbj_sheet.T8FromBottom_mm_(row_num);
...
%     end
    ...
    if flags.solve_problem == 1
        ...
        mesh = ParForwardMesh1st(nodes*1e-3, connectivity, E_connect);
...
%     solver.zeta = zeta(i)*ones(L,1);
    ...
    if flags.solve_problem == 1
        ...
        % Solve the voltage
        solve_start = tic;
        [Umeas, Imeas, Uall] = MF_Simulation(mesh, [], sigma, solver.zeta, solver, 'current', err);
