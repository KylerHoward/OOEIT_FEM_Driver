...
%{
Editted to incorporate matrix free methods
Editted: 10/10/24 - Kyler Howard
%}
    ...
    properties (SetObservable) 
        %{
        This block has to feature all the properties that affect the S and 
        b parts of solving FEM. That way, we can listen to changes and 
        recalculate parts of the FEM matrix only when necessary.
        %}
    ...
    methods
        ...
        function SetInvGamma(self, constNoise, relNoise)
            ...
            Input: const_noise = a constant noise level for all
                   measurements. This noise is scaled to the difference of 
        ...
        function J = Jacobian(self, sigma, alreadyComputed)
            ...
            % indices with non-zero entries, for computational performance, 
            % the calculations have been split in the following way.
            ...
            selfAi = parallel.pool.Constant(self.Ai);
            selfAv = parallel.pool.Constant(self.Av);
        ...
        function Js = JacobianZ(self, est, alreadyComputed)
            ...
            corresponds to the conductivity given. Default is to recompute
            to make sure.
            %}
            if nargin < 3 || isempty(alreadyComputed)
                alreadyComputed = 0;
            ...
            par_zeta = parallel.pool.Constant(self.zeta);
            par_intM = parallel.pool.Constant(self.intM);
            par_intS = parallel.pool.Constant(self.intS);
            par_intB = parallel.pool.Constant(self.intB);
            par_mode = parallel.pool.Constant(self.mode);
            par_zInd = parallel.pool.Constant(self.zInd);
            par_C    = parallel.pool.Constant(self.C);
            par_nEl  = parallel.pool.Constant(self.fmesh.nEl);
